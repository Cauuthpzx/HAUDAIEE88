...
router.post('/login', (req, res) => {
  ...
  if (!validator.isLength(username, { min: 2, max: 50 })) {
    return res
      .status(400)
      .json({ code: -1, msg: 'Tên đăng nhập không hợp lệ' });
  }

  const db = getDb();
  const user = db
    .prepare('SELECT * FROM hub_users WHERE username = ? AND status = 1')
    .get(username);
  ...
  if (
    ...
    !bcrypt.compareSync(
      password,
      user ? user.password_hash : '$2a$10$invalidhashplaceholderxxx'
    )
  ) {
    log.warn(
      `Đăng nhập thất bại: ${!user ? 'user không tồn tại' : 'sai mật khẩu'} (${username})`,
      { ip: req.ip }
    );
    return res
      .status(401)
      .json({ code: -1, msg: 'Tên đăng nhập hoặc mật khẩu không đúng' });
  }

  const ek = crypto.randomBytes(32).toString('hex');
  ...
  log.ok(`Đăng nhập thành công: ${username} (role: ${user.role})`, {
    ip: req.ip
  });
  logActivity({
    userId: user.id,
  ...
  // Cảnh báo nếu cần đổi mật khẩu mặc định
...
router.get('/me', authMiddleware, (req, res) => {
  ...
  const user = db
    .prepare(
      'SELECT id, username, display_name, role, created_at FROM hub_users WHERE id = ?'
    )
    .get(req.user.id);
...
router.post('/change-password', authMiddleware, (req, res) => {
  ...
  if (!validator.isLength(new_password, { min: 6, max: 128 })) {
    return res
      .status(400)
      .json({ code: -1, msg: 'Mật khẩu mới phải từ 6-128 ký tự' });
  }

  const db = getDb();
  const user = db
    .prepare('SELECT * FROM hub_users WHERE id = ?')
    .get(req.user.id);
  ...
  db.prepare(
    "UPDATE hub_users SET password_hash = ?, must_change_password = 0, updated_at = datetime('now', 'localtime') WHERE id = ?"
  ).run(hash, user.id);
...
router.post('/logout-all', authMiddleware, (req, res) => {
  const db = getDb();
  db.prepare(
    "UPDATE hub_users SET token_version = token_version + 1, updated_at = datetime('now', 'localtime') WHERE id = ?"
  ).run(req.user.id);
