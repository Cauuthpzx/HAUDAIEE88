...
router.get('/activity-log', (req, res) => {
  ...
  const total = db
    .prepare(`SELECT COUNT(*) as cnt FROM hub_activity_log WHERE ${where}`)
    .get(...params).cnt;
  const rows = db
    .prepare(
      `
...
router.post('/agents/login-all', async (req, res) => {
  ...
  if (!isSolverReady()) {
    return res.json({
      code: -1,
      msg: 'OCR engine chưa sẵn sàng, vui lòng đợi khởi tạo'
    });
  }
  ...
  if (agents.length === 0) {
    return res.json({
      code: 0,
      msg: 'Không có agent nào cần login',
  ...
  res.json({
    code: 0,
    msg: `Login xong: ${success} OK, ${fail} thất bại`,
...
router.get('/agents', (req, res) => {
  ...
  agents.forEach((a) => {
    if (a.cookie) {
      ...
      a.cookie_preview = sessId
        ? `PHPSESSID=${sessId.substring(0, 8)}...`
        : '***';
    }
...
router.post('/agents', async (req, res) => {
  ...
  try {
    ...
    if (loginResult.success) {
      ...
      const autoCreateUser = db.transaction(() => {
        ...
        if (!existingUser) {
          ...
          ).run(userResult.lastInsertRowid, agentId);
          log.ok(
            `Auto-tạo user "${ee88_username}" (id=${userResult.lastInsertRowid}) gắn agent #${agentId}`
          );
        }
      });
      autoCreateUser();
      res.json({
        code: 0,
        msg: `Đã thêm + login thành công (${loginResult.attempts} lần thử)`,
        data: { id: agentId }
      });
    } else {
      res.json({
        code: 0,
        msg: `Đã thêm agent, login thất bại: ${loginResult.error}`,
        data: { id: agentId }
      });
    }
  } catch (err) {
    res.json({
      code: 0,
      msg: `Đã thêm agent, login lỗi: ${err.message}`,
      data: { id: agentId }
    });
  }
...
router.put('/agents/:id', (req, res) => {
  ...
  const newPassword = ee88_password
    ? encrypt(ee88_password)
    : agent.ee88_password || '';
...
router.post('/agents/:id/check', async (req, res) => {
  ...
  try {
    ...
    if (response.data && response.data.url === '/agent/login') {
      db.prepare(
        "UPDATE ee88_agents SET last_check = datetime('now', 'localtime'), status = 0, updated_at = datetime('now', 'localtime') WHERE id = ?"
      ).run(id);
      adminEmitter.emit('change', {
        type: 'agent',
        action: 'check',
    ...
    db.prepare(
      "UPDATE ee88_agents SET last_check = datetime('now', 'localtime'), status = 1, updated_at = datetime('now', 'localtime') WHERE id = ?"
    ).run(id);
    adminEmitter.emit('change', {
      type: 'agent',
      action: 'check',
...
router.post('/agents/:id/login', async (req, res) => {
  ...
  if (!agent.ee88_username || !agent.ee88_password) {
    return res.json({
      code: -1,
      msg: 'Chưa cấu hình username/password EE88 cho agent này'
    });
  }
  ...
  if (result.success) {
    logActivity({
      userId: req.user.id,
      username: req.user.username,
      action: 'agent_login_success',
      targetType: 'agent',
      targetId: parseInt(id),
      targetLabel: agent.label,
      detail: `${result.attempts} attempts`,
      ip: req.ip
    });
    adminEmitter.emit('change', {
      type: 'agent',
    ...
    res.json({
      code: 0,
      msg: `Login thành công (${result.attempts} lần thử)`,
      attempts: result.attempts
    });
  } else {
    logActivity({
    ...
    res.json({
      code: -1,
      msg: result.error || 'Login thất bại',
...
router.get('/solver-status', (req, res) => {
  res.json({
    code: 0,
    data: { running: isSolverReady(), engine: 'tesseract.js' }
  });
});
...
router.get('/data-store/:endpoint', (req, res) => {
  ...
  const { agent_id, date_key, page, limit, search, order_by, order } =
    req.query;
...
router.post('/users', (req, res) => {
  ...
  if (!username || !password) {
    return res
      .status(400)
      .json({ code: -1, msg: 'Thiếu tên đăng nhập hoặc mật khẩu' });
  }
  ...
  if (!validator.isLength(password, { min: 6, max: 128 })) {
    return res
      .status(400)
      .json({ code: -1, msg: 'Mật khẩu phải từ 6-128 ký tự' });
  }
  if (role && !['admin', 'user'].includes(role)) {
    return res.status(400).json({ code: -1, msg: 'Role không hợp lệ' });
  }
  if (display_name)
    display_name = validator.trim(validator.escape(display_name));
...
router.put('/users/:id', (req, res) => {
  ...
  const updateUser = db.transaction(() => {
    ...
    if (password && password.length >= 6) {
      const hash = bcrypt.hashSync(password, 10);
      db.prepare(
        "UPDATE hub_users SET password_hash = ?, updated_at = datetime('now', 'localtime') WHERE id = ?"
      ).run(hash, id);
    }
    ...
    if (Array.isArray(agent_ids)) {
      db.prepare('DELETE FROM user_agent_permissions WHERE user_id = ?').run(
        id
      );
      const insertPerm = db.prepare(
        'INSERT INTO user_agent_permissions (user_id, agent_id) VALUES (?, ?)'
