...
router.get('/agents', (req, res) => {
  ...
  // Ẩn cookie trong response list (chỉ hiện 1 phần)
  agents.forEach(a => {
    if (a.cookie) {
...
router.post('/agents', (req, res) => {
  const { label, base_url, cookie } = req.body;

  if (!label || !base_url || !cookie) {
    return res.status(400).json({ code: -1, msg: 'Thiếu thông tin (label, base_url, cookie)' });
  }

  const db = getDb();
  const result = db.prepare(
    'INSERT INTO ee88_agents (label, base_url, cookie) VALUES (?, ?, ?)'
  ).run(label, base_url, cookie);

  log.ok(`Thêm agent: ${label} (id=${result.lastInsertRowid})`);
  res.json({ code: 0, msg: 'Đã thêm agent', data: { id: result.lastInsertRowid } });
});

// PUT /api/admin/agents/:id — Sửa agent
router.put('/agents/:id', (req, res) => {
  const { label, base_url, cookie, status } = req.body;
  ...
  db.prepare(`
    UPDATE ee88_agents
    SET label = ?, base_url = ?, cookie = ?, status = ?, updated_at = datetime('now', 'localtime')
    WHERE id = ?
  `).run(
    label || agent.label,
