...
function getKey() {
  ...
  if (!keyHex || keyHex.length !== 64) {
    throw new Error('ENCRYPTION_KEY chưa được cấu hình hoặc không hợp lệ (cần 64 hex chars)');
  }
...
function ensureEncryptionKey() {
  ...
  try {
    ...
    log.warn('Hãy backup giá trị ENCRYPTION_KEY này! Mất key = mất dữ liệu mã hóa.');
  } catch (err) {
...
function isEncrypted(text) {
  ...
  return /^[0-9a-f]+$/.test(parts[0]) && /^[0-9a-f]+$/.test(parts[1]) && parts[1].length >= 32;
}

module.exports = { encrypt, decrypt, isEncrypted, ensureEncryptionKey };
