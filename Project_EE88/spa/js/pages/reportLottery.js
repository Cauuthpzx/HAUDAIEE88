(function () {
  // Lottery options
  var lotteryOpts = '<option value="" data-i18n="selectOrSearch">Chọn hoặc nhập để tìm kiếm</option>'
  ...
  SpaPages.reportLottery = {
    getHTML: function () {
      return '<div class="layui-row"><div class="layui-col-md12"><div class="layui-card">'
        ...
        + '<input type="text" name="date" id="rl_dateRange" placeholder="' + HubLang.t('dateStartEnd') + '" class="layui-input" readonly autocomplete="off"></div></div>'
        ...
        + '<select name="lottery_id" lay-search="">' + lotteryOpts + '</select></div></div>'
    ...
    init: function (container) {
      ...
      laydate.render({ elem: '#rl_dateRange', type: 'date', range: '|', rangeLinked: true, max: 0, value: defaultRange });
      ...
      function renderTotalData(totalData) {
        if (!totalData) return;
        var fields = ['rl_total_bet_number', 'rl_total_bet_count', 'rl_total_bet_amount', 'rl_total_valid_amount', 'rl_total_rebate_amount', 'rl_total_result', 'rl_total_win_lose', 'rl_total_prize'];
        fields.forEach(function (key) {
          var dataKey = key.replace('rl_', '');
          var val = totalData[dataKey];
          if (val === undefined || val === null) {
            val = (dataKey === 'total_bet_count' || dataKey === 'total_bet_number') ? '0' : '0.0000';
          }
      ...
      $(container).find('#rl_btnReset').on('click', function () {
        setTimeout(function () {
          $('#rl_dateRange').val(defaultRange);
          form.render('select');
          table.reload('rl_dataTable', {
            where: { date: defaultRange, username: '', lottery_id: '' },
            page: { curr: 1 }
          });
        }, 50);
      });
